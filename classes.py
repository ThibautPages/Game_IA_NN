...
import copy
import random
from intelligenceArtificielle import *
...
class Table:
    ...
    def __init__(self, longueur, largeur):
        ...
        self.poidsCases = [[0.5 for i in range(largeur+1)] for j in range(longueur+1)]

        for i in range(longueur):
            self.poidsCases[i][largeur] = 0.01

        self.poidsCases[longueur] = [0.01 for i in range(largeur+1)]
...
class Maison (Objet):
    ...
    def __init__(self,table,nom,ligne,colonne):
        ...
        self.nourriture = 0
        self.minerai = 0
        table.poidsCases[ligne-1][colonne-1] = 0.7
...
class Ouvrier (Objet):
    ...
    def action(self):
        # tableau contenant les poids des cases entourant l'ouvrier
        _,ligne,colonne = self.table.dictionnaireObjet[self.nomObjet]
        cases = [self.table.poidsCases[ligne - 1 + (i%3 -1)][colonne - 1 + (i//3 -1)] for i in range(9)]

        depl = self.ia.deplacement(cases)
...
class Mine (Objet):
    def __init__(self,table,nom,ligne,colonne):
        Objet.__init__(self,table,nom,ligne,colonne)
        table.poidsCases[ligne-1][colonne-1] = 0.99
